        ...
        async for message in websocket:
            ...
            if action == "join_queue":
                ...
                if len(waiting_queue) > 0:
                    opponent_ws, opponent_id = waiting_queue.pop(0)
                    ...
                    waiting_queue.append((websocket, player_id))
                    await websocket.send(json.dumps({
                        "action": "waiting",
                ...
                if current_battle and current_battle in active_battles:
                    ...
                    target_side = "player2" if player_side == "player1" else "player1"
                    target_ws = battle.get(target_side)
                ...
                if (websocket, player_id) in waiting_queue:
                    waiting_queue.remove((websocket, player_id))
            
            elif action == "leave_battle":
                if current_battle and current_battle in active_battles:
                    battle = active_battles[current_battle]
                    other_side = "player2" if player_side == "player1" else "player1"
                    other_ws = battle.get(other_side)
                    if other_ws:
                        try:
